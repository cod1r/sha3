...
pub fn keccak(allocator: std.mem.Allocator, rate: usize, in: []const u8, digest_len: usize, lastbyte: u8) ![]u8 {
    ...
    var outputBytes = std.ArrayList(u8).init(allocator);
...
test "sha3 224" {
    ...
    const in: []const u8 = "";
    const out: []u8 = try keccak(alloc, b - 2 * 224, in, 224 / 8, 0x06);
...
test "sha3 256" {
    ...
    const in: []const u8 = "";
    const out: []u8 = try keccak(alloc, b - 2 * 256, in, 256 / 8, 0x06);
    defer alloc.free(out);
...
test "sha3 384" {
    ...
    const in: []const u8 = "";
    const out: []u8 = try keccak(alloc, b - 2 * 384, in, 384 / 8, 0x06);
    defer alloc.free(out);
...
test "sha3 512" {
    ...
    const in: []const u8 = "";
    const out: []u8 = try keccak(alloc, b - 2 * 512, in, 512 / 8, 0x06);
    defer alloc.free(out);
...
test "shake 128" {
    ...
    const in: []const u8 = "";
    const out: []u8 = try keccak(alloc, b - 2 * 128, in, 256 / 8, 0x1F);
    defer alloc.free(out);
    try std.testing.expect(out.len == 256 / 8);
    var index: usize = 0;
    for (out) |val| {
...
test "shake 256" {
    ...
    const in: []const u8 = "";
    const out: []u8 = try keccak(alloc, b - 2 * 256, in, 512 / 8, 0x1F);
    defer alloc.free(out);
    try std.testing.expect(out.len == 512 / 8);
    var index: usize = 0;
    for (out) |val| {
